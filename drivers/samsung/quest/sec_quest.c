...
static int do_quest()
{
	...
	switch (param_quest_data.curr_step) {
		...
		case STEP_CALX:
			...
			argv[1] = log_path;
			argv[2] = "Reboot";
#if 1	// temporarily disable naturescene at CAL QUEST
			argv[3] = "gfxTestEnabled:0\0";
			QUEST_PRINT("gfxTestEnabled option disabled \n");
#endif
			QUEST_PRINT("reboot option enabled \n");
			break;
		case STEP_TESTMODE:
...
//////////////////////////////////////////////////
static ssize_t store_quest_naturescene_info(struct device *dev,
			      struct device_attribute *attr,
			      const char *buf, size_t count)
{
	int idx = 0, naturescene_tested_times = 0, naturescene_total_elapsed_time = 0;
	char temp[QUEST_BUFF_SIZE * 3];
	char quest_cmd[QUEST_CMD_LIST][QUEST_BUFF_SIZE];
	char *quest_ptr, *string;
	...
	if( param_quest_data.curr_step != STEP_SMDDL ) {
		QUEST_PRINT("%s : The naturescene info should be updated only at SMDDL\n", __func__);
		goto out;
	}
	...
	sscanf(quest_cmd[0], "%d", &naturescene_tested_times);
	sscanf(quest_cmd[1], "%d", &naturescene_total_elapsed_time);

	param_quest_data.naturescene_tested_times = naturescene_tested_times;
	param_quest_data.naturescene_total_elapsed_time = naturescene_total_elapsed_time;
...
static DEVICE_ATTR(nad_naturescene_info, S_IWUSR, NULL, store_quest_naturescene_info);
//////////////////////////////////////////////////
...
static int __init sec_quest_init(void)
{
	...
	ret = device_create_file(sec_nad, &dev_attr_nad_naturescene_info);
	if (ret) {
		QUEST_PRINT("%s: Failed to create device file\n", __func__);
		goto err_create_nad_sysfs;
	}
