...
#define SAMSUNG_QMI_WAIT_MS (1000)
#endif
...
#if defined(CONFIG_USB_HOST_SAMSUNG_FEATURE)
			uaudio_dbg("sent qmi send msg");
			ret = wait_event_interruptible_timeout(dev->disconnect_wq,
					!atomic_read(&dev->in_use), SAMSUNG_QMI_WAIT_MS);
			uaudio_dbg("in_use=%d, retry_count=%d, time=%d\n", atomic_read(&dev->in_use), retry_count, ret);
			if(atomic_read(&dev->in_use) == 0)
				break;			
		}
		
		if(atomic_read(&dev->in_use)) {
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
			panic("can't close audio stream! force panic!");
#else
